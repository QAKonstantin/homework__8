...
parser.add_argument('-f', dest='path', action='store', default=f"{os.getcwd()}/logs/", help='Path to logs')
args = parser.parse_args()

dict_ip = defaultdict(
    lambda: {"COUNT": 0, "GET": 0, "POST": 0, "PUT": 0, "DELETE": 0, "HEAD": 0, "PATCH": 0}
)
...
def logs_analyzing(filename):
    with open(filename, 'r') as file:
        for line in file.readlines():
            ...
            method_match = re.search(r"POST|GET|PUT|DELETE|HEAD|PATCH", line)
